...
3. **Ollama / OpenAI-compatible endpoint**: For running local or hosted LLMs.
   ...
   local Ollama daemon (`ollama serve`, then `ollama pull deepseek-v4-flash`)
   for a local/offline build, or point `LLM_API_BASE` at any OpenAI-compatible
   server (e.g. Ollama Cloud, `https://ollama.com/v1`). No cloud SDK or account
   credentials are required for the default local-Ollama path. **If using
   hosted/`:cloud` models**, provide an API key in a git-ignored `.env` file
   (template: `reference/.env.example`), e.g. `OLLAMA_API_KEY=your-key` for
   Ollama Cloud.
